...
function getToneLabel(m: MemberLanguageProfile): { label: string; colorClass: string } {
  if (m.modalParticles.length === 0)
    return { label: t('quotes.languagePreference.modal.toneMixed'), colorClass: 'text-gray-600 dark:text-gray-400' }
  let soft = 0,
    curious = 0,
    casual = 0,
    calm = 0,
  ...
  if (max === soft && soft / total > 0.3)
    return { label: t('quotes.languagePreference.modal.toneSoft'), colorClass: 'text-pink-600 dark:text-pink-400' }
  ...
  if (max === calm && calm / total > 0.3)
    return { label: t('quotes.languagePreference.modal.toneCalm'), colorClass: 'text-blue-600 dark:text-blue-400' }
  return { label: t('quotes.languagePreference.modal.toneMixed'), colorClass: 'text-gray-600 dark:text-gray-400' }
}
